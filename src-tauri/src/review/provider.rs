...
{}",
    ...
    #[test]
    fn the_prompt_plan_gate_accepts_the_portuguese_alias_shape() {
        // Regressao do erro reportado: o qwen local respondia com
        // perguntas/pergunta/dica e o gate estrito rejeitava com
        // "/prompts: campo obrigatorio ausente". O schema do plano de questoes
        // e tolerante; a validacao semantica acontece em parse_prompt_plan.
        use crate::review::schema::validate_instance;
        use crate::review::session::prompt_plan_schema;
        let schema = prompt_plan_schema();

        let aliased = json!({
            "perguntas": [
                { "pergunta": "O que e a fotolise?", "dica": "Que molecula e quebrada?", "alternativas": ["A", "B", "C", "D"], "respostaCorreta": 1 },
                { "pergunta": "Qual o papel da fase clara?", "dica": "Que produtos gera?", "alternativas": ["A", "B", "C", "D"], "respostaCorreta": 0 },
                { "pergunta": "O que a fase escura produz?", "dica": "Que molecula e usada?", "alternativas": ["A", "B", "C", "D"], "respostaCorreta": 2 }
            ]
        });
        assert!(
            validate_instance(&schema, &aliased).is_empty(),
            "o gate deve aceitar a forma com aliases"
        );

        let canonical = json!({
            "prompts": [{ "text": "Pergunta", "assistance": "Dica", "options": ["A", "B", "C", "D"], "correctOptionIndex": 1 }]
        });
        assert!(validate_instance(&schema, &canonical).is_empty());

        let wrong_type = json!({"prompts": "nao-e-uma-lista"});
        assert!(!validate_instance(&schema, &wrong_type).is_empty());
    }

    #[test]
    fn the_evaluation_gate_accepts_the_per_question_alias_shape() {
        // Regressao do erro reportado na correcao: o qwen respondeu com um
        // array por pergunta (questions/lacunas) no lugar do contrato agregado
        // e o gate estrito rejeitava. O schema da avaliacao e tolerante; a
        // validacao semantica acontece em parse_review_evaluation.
        use crate::review::schema::validate_instance;
        use crate::review::session::review_evaluation_schema;
        let schema = review_evaluation_schema();

        let per_question = json!({
            "questions": [
                { "promptId": "question-1", "score": 83, "lacunas": ["Onde ocorre a fotolise? D) Cloroplasto"] },
                { "promptId": "question-2", "score": 100 },
                { "promptId": "question-3", "score": 83, "lacunas": ["Qual produto nao e formado? C) NADPH"] }
            ]
        });
        assert!(
            validate_instance(&schema, &per_question).is_empty(),
            "o gate deve aceitar a avaliacao por pergunta"
        );

        let aliased = json!({
            "nota": 86,
            "resumo": "Bom dominio.",
            "lacunas": [{ "classificacao": "esquecido", "citacao": "energia luminosa" }]
        });
        assert!(validate_instance(&schema, &aliased).is_empty());

        let canonical = json!({
            "score": 86,
            "summary": "Bom dominio.",
            "gaps": [{ "classification": "forgotten", "sourceQuote": "energia luminosa" }]
        });
        assert!(validate_instance(&schema, &canonical).is_empty());

        let wrong_type = json!({"score": "cem"});
        assert!(!validate_instance(&schema, &wrong_type).is_empty());
    }
