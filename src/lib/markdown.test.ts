...
describe('findMarkdownWordAtOffset', () => {
  it('returns the non-whitespace token touching the cursor', () => {
    expect(findMarkdownWordAtOffset('**negrito** e italico', 5)).toBe('**negrito**')
    expect(findMarkdownWordAtOffset('**negrito** e italico', 0)).toBe('**negrito**')
    expect(findMarkdownWordAtOffset('**negrito** e italico', 15)).toBe('italico')
    expect(findMarkdownWordAtOffset('texto [[nota]] fim', 8)).toBe('[[nota]]')
    expect(findMarkdownWordAtOffset('texto `codigo` fim', 8)).toBe('`codigo`')
  })

  it('includes the block marker for the first word of a marked line', () => {
    expect(findMarkdownWordAtOffset('# Titulo', 5)).toBe('# Titulo')
    expect(findMarkdownWordAtOffset('- item', 3)).toBe('- item')
    expect(findMarkdownWordAtOffset('> citacao', 4)).toBe('> citacao')
    expect(findMarkdownWordAtOffset('1. item', 4)).toBe('1. item')
    expect(findMarkdownWordAtOffset('[x] tarefa', 5)).toBe('[x] tarefa')
  })

  it('handles the cursor over the marker and plain words', () => {
    expect(findMarkdownWordAtOffset('# Titulo', 1)).toBe('#')
    expect(findMarkdownWordAtOffset('Texto normal aqui', 6)).toBe('normal')
    expect(findMarkdownWordAtOffset('Texto normal aqui', 0)).toBe('Texto')
    expect(findMarkdownWordAtOffset('', 0)).toBe('')
  })
