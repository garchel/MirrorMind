...
export function diffVaultNotePaths(previousPaths: string[], nextPaths: string[]) {
  const previous = new Set(previousPaths)
  const next = new Set(nextPaths)
  const removedPaths = previousPaths.filter((path) => !next.has(path))
  const createdPaths = nextPaths.filter((path) => !previous.has(path))
