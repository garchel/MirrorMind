...
// Retorna a palavra/token Markdown colado ao cursor em `offset`, para exibir a
// forma crua do elemento sob o cursor (ex.: "**negrito**", "[[nota]]",
// "# Titulo", "- item"). Marcadores de bloco (heading, citacao, lista, tarefa)
// sao incluidos quando o cursor esta sobre a primeira palavra da linha.
export function findMarkdownWordAtOffset(markdown: string, offset: number, maxLength = 32): string {
  ...
  const inLine = position - lineStart

  const markerMatch = line.match(/^\s{0,3}(#{1,6}\s+|>+\s*|[-*+]\s+|\d+[.)]\s+|\[[ xX]\]\s+)/)
  const marker = markerMatch ? markerMatch[0] : ''

  // Cursor sobre o proprio marcador (ex.: entre o "#" e o texto).
  if (marker && inLine < marker.length) return marker.trimEnd().slice(0, maxLength) || ''

  const before = content.slice(lineStart, position)
  const after = content.slice(position, lineEnd)
  let start = before.length
  while (start > 0 && !/\s/.test(before[start - 1])) start -= 1
  let end = 0
  while (end < after.length && !/\s/.test(after[end])) end += 1
  let word = `${before.slice(start)}${after.slice(0, end)}`

  if (marker && start <= marker.length) {
    const afterMarker = line.slice(marker.length)
    const firstWord = afterMarker.match(/^\S+/)?.[0] ?? ''
    if (firstWord && word.startsWith(firstWord)) {
      word = `${marker.trimEnd()} ${firstWord}`.trim()
    }
  }
