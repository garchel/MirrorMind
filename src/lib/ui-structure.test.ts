...
describe('Estrutura de UI e nomes amigaveis', () => {
  it('nomeia a janela do editor (busca Ctrl+F e popover de selecao)', () => {
  ...
  it('nomeia o cabecalho da nota (titulo, avaliacao, modo de visualizacao e tags)', () => {
    expect(UI_STRUCTURE.workspace.noteHeader).toBe('editor-header')
    expect(BUILDER_FRIENDLY_NAMES['editor-header']).toBe('Cabecalho da nota')
  })
})
