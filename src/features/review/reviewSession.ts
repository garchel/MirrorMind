...
const reviewPromptSchema = z.object({
  ...
  // Alternativas de multipla escolha; vazio no modo conversa. A alternativa
  // correta nunca trafega para o cliente: o backend guarda e corrige.
...
const reviewSessionDraftSchema = z.object({
  ...
  const validExam = draft.mode === 'exam'
    ...
    && draft.prompts.every((prompt) => prompt.options.length >= 3 && prompt.options.length <= 5)
...
const reviewCompletionReportSchema = z.object({
  ...
  const roundedMean = Math.round(report.units.reduce((sum, unit) => sum + unit.score, 0) / report.units.length)
  if (roundedMean !== report.overallScore) {
    issue('The overall score must equal the rounded mean of the unit scores.')
  }
