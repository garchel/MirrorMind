...
describe('NoteReviewPolicyControl', () => {
  ...
  it('details the origin of every effective field', async () => {
    const user = userEvent.setup()
    getPolicyMock.mockResolvedValueOnce({
      ...policy,
      targetRetention: 0.9,
      deadlineAtUnixMs: 1_930_000_000_000,
      sources: {
        ...policy.sources,
        targetRetention: { kind: 'note', sourceId: 'note-1' },
        priorityWeight: { kind: 'activeDeadlineTag', sourceId: 'prova-bio' },
        deadlineAtUnixMs: { kind: 'activeDeadlineTag', sourceId: 'prova-bio' },
        activeDeadline: { kind: 'activeDeadlineTag', sourceId: 'prova-bio' },
      },
    })
    render(<NoteReviewPolicyControl
      vaultPath={'C:\\Vault'}
    ...
    expect(screen.getByText('Origem de cada campo')).toBeInTheDocument()
    expect(screen.getByText('Configuração da nota')).toBeInTheDocument()
    expect(screen.getAllByText('Padrão do Vault')).toHaveLength(3)
    expect(screen.getAllByText('Prazo ativo · #prova-bio')).toHaveLength(2)
    expect(screen.getByText('90%')).toBeInTheDocument()
  })

  it('shows an expired deadline origin for the exposed deadline', async () => {
    const user = userEvent.setup()
    getPolicyMock.mockResolvedValueOnce({
      ...policy,
      deadlineAtUnixMs: Date.now() - 86_400_000,
      sources: {
        ...policy.sources,
        firstReviewIntervalDays: { kind: 'expiredDeadlineTag', sourceId: 'prova-antiga' },
        targetRetention: { kind: 'expiredDeadlineTag', sourceId: 'prova-antiga' },
        priorityWeight: { kind: 'expiredDeadlineTag', sourceId: 'prova-antiga' },
        minIntervalDays: { kind: 'expiredDeadlineTag', sourceId: 'prova-antiga' },
        maxIntervalDays: { kind: 'expiredDeadlineTag', sourceId: 'prova-antiga' },
        deadlineAtUnixMs: { kind: 'expiredDeadlineTag', sourceId: 'prova-antiga' },
        activeDeadline: null,
      },
    })
    render(<NoteReviewPolicyControl
    ...
    expect(screen.getAllByText('Prazo encerrado · #prova-antiga')).toHaveLength(6)
