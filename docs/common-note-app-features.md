...
| Links internos no motor único | Implementado | Wikilinks e links Markdown são clicáveis no modo Leitura (motor único, Marco 10 do unified-reader-roadmap) e revelam o Markdown cru com o cursor por perto no modo Misto. O tooltip de hover com resumo da nota (exclusivo do renderer clássico aposentado) foi removido de propósito — a navegação acontece no clique. |
